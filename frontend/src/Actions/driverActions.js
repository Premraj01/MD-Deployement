...
export const registerDriver = (
  ...
  try {
    ...
    const { data } = await axios.post(
      `${AppConst.BASE_URL}/api/admin/drivers`,
      {
        firstName,
        lastName,
        mobileNumber,
...
export const getDriversList = (keyword = "", pageNumber = "") => async (
  dispatch,
  getState,
) => {
  try {
    ...
    const { data } = await axios.get(
      `${AppConst.BASE_URL}/api/admin/drivers?keyword=${keyword}`,
      config,
    );
...
export const getDriverDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(
      `${AppConst.BASE_URL}/api/admin/drivers/${id}`,
      config,
    );
...
export const updateDriver = (driver) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `${AppConst.BASE_URL}/api/admin/drivers/${driver._id}`,
      driver,
      config,
    );
